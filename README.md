...
传递视频组数组给回调：
[
  { blob, type: 'before-speaking', duration: 5000ms },  ← 说话前的组（已完成）
  { blob, type: 'before-speaking', duration: 2000ms },  ← 说话前的组（正在录制）
  { blob, type: 'speaking', duration: 3000ms }          ← 说话期间的组
]
    ↓
清空已捕获的视频组（防止重复）
```
...
- **特殊值**：
  - 设置为 `0`：完全依赖动态计算，不设置最小值限制
  - 不设置（undefined）：使用默认值 20
- **调整建议**：
  - 完全依赖动态计算：0
  - 更敏感（容易触发）：15 - 18
  - 默认（推荐）：20
